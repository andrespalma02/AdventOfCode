...
#Guardamos cada linea como un elemento de una lista
lines = f.readlines()
...
def primera_parte(lines):
    ...
    while True:
        # Suponiendo que recorrimos todas las lineas y no se encontraron bucles
        if len(lines)-1 == contador_por_linea:
            solucion_valida = False

        # Solucion de la primera parte para eliminar los bucles
        if contador_por_linea in linea_visitada: #iteramos por cada linea visitada
            solucion_valida = False #buscando una solucion valida a cada una de las instrucciones
        ...
        # si encuentra una operacion nop el contador por linea aumenta en 1 para que lea
        ...
        if inst == "acc":
            ...
            contador_por_linea += 1
        # si encuentra una operacion jump el contador por linea aumenta tantas veces como esta
        # definida en la operacion jmp y empieza a leer la linea definida por jmp
        if inst == "jmp":
...
def segunda_parte(lines):
    ...
    #recorremos la lista por cada una de las lineas nuevas
